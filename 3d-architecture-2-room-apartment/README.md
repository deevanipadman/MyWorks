...
Most reliable (recommended): serve locally, then open in your browser.

### Option A — Local server on Windows PowerShell
```powershell
...
You can try double‑clicking `index.html` to open via file://. If your browser blocks ES module import maps on file URLs, use Option A.
...
├── README.md                  # Quick overview and run instructions
├── Requirement.txt            # Assignment requirements
└── Final_Project_Report.md    # Detailed report (3–5 pages)
```
...
- **Show/Hide Front Wall**: Toggle front wall visibility for inspection

### Keyboard
- The OrbitControls support arrow keys for panning (optional)

<!-- Note: Removed an old/deprecated section that referenced package.json and css/js folders.
The project is intentionally single-file (index.html) plus docs. -->
